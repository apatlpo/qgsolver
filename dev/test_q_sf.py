...
if __name__ == "__main__":
    ...
    if old :
        ...
        psiin = Dataset(psi_file, 'r')
        psih = psiin.variables['psi0_hydrostatic'][level-1:level+2,ys:ye,xs:xe]    
        psi_surf = psiin.variables['psi0_surface_pressure'][ys:ye,xs:xe]
        psi=np.zeros_like(psih[:,:,:])
        psi[:,:,:] = psih[:,:,:] + psi_surf[None,:,:]
    ...
    for j in range(1,M-1):
        for i in range(1,L-1):
            q_stretch[j,i] = ( f0**2/N2[k+1] * (psih[k+1,j,i]-psih[k,j,i])/e3f[k+1] - \
                               f0**2/N2[k] * (psih[k,j,i]-psih[k-1,j,i])/e3f[k]) / e3t[k]
    ...
    # store original PV 
    dtype='f8'
    pv1 = create_nc('data/pv_orig_accurate_fprime_ok_test_5.nc', lon, lat)
    nc_q1 = pv1.createVariable('q_orig',dtype,('y','x'))
    nc_q1[:] = q[k,:,:]

    # store calculated PV 
    pv2 = create_nc('data/pv_calc_accurate_fprime_ok_test_5.nc', lon, lat)
    nc_q2 = pv2.createVariable('q_calc',dtype,('y','x'))
    nc_q2[:] = q_sum

    # store difference PV 
    pv3 = create_nc('data/pv_diff_accurate_fprime_ok_test_5.nc', lon, lat)
